...
sdata1 <- read_csv("C:/.../data.csv",
                   col_names = FALSE, col_types = cols(X1 = col_character(),
                                                       X2 = col_double()))

# read projection recovery map for a corresponding dataset
results <- read.csv("PRM_b_1.csv")
...
# A random dataset of 288 data points is used, that is, from input dataset between chosen indexes 
min_index <- 7744
max_index <- 8031
input <- input[min_index:max_index,] 
origin <- ts(input[2])
plot(input)

# Making artificial multiple gaps, imitating real-world scenarios
input[15,2] <- NA
input[56:57,2] <- NA
input[70:86,2] <- NA
input[198:227,2] <- NA
...
z <- fill_gaps(y, origin, sdata1, min_index)
